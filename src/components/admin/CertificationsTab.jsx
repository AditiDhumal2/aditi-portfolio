...
const CertificationsTab = ({ certifications, setCertifications, showMessage, setUploading, uploading, fetchAllData }) => {
  ...
  const addCertification = async () => {
    ...
    try {
      ...
      console.log('Added certification:', response.data);
      await fetchAllData();
      showMessage('Certification added successfully!');
    } catch (error) {
        ...
        {certifications && certifications.length > 0 ? (
          certifications.map(cert => (
            <div key={cert._id} className="bg-gray-800 rounded-xl p-4 border border-gray-700">
              ...
              {/* Display current certificate image - IMPROVED VISIBILITY */}
              {cert.image && editingCert !== cert._id && (
                <div className="mb-4">
                  <label className="block text-sm font-semibold text-gray-400 mb-1">Certificate Image</label>
                  <div className="bg-gray-700 p-2 rounded-lg inline-block">
                    <img 
                      src={cert.image} 
                      alt={cert.name} 
                      className="w-40 h-40 object-contain rounded-lg"
                      onError={(e) => {
                        e.target.src = 'https://via.placeholder.com/160?text=Image+Not+Found';
                      }}
                    />
                  </div>
                </div>
